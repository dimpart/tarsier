...
void changeRootPage(BuildContext context, WidgetBuilder builder) {
...
void changeToMainPage(BuildContext context) =>
    changeRootPage(context, (context) => const _MainPage());
...
class _MainPageState extends State<_MainPage> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) => _systemCheck(context));
  }

  Future<void> _systemCheck(BuildContext context) async {
    await Future.delayed(const Duration(milliseconds: 200));
    if (context.mounted) {
      // system check
      await _SystemChecker().check(context);
    }
  }
